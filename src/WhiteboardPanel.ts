	...
	<body>
		...
		<div class="toolbar">
			...
			<div class="tool-group">
				...
				<button class="tool-btn" onclick="exportImage()" title="Export">
					💾
				</button>
			</div>
		</div>
		...
		<script>
			...
			function createColorPalette() {
				...
				if (!paletteContainer) {
					console.warn(
						"Color palette container not found. Add <div id='colorPalette'></div> to your HTML"
					);
					return;
				}
				...
				paletteContainer.style.boxShadow =
				...
				colorPalette.forEach((color) => {
					...
					if (
						color === currentColor ||
						color === currentColor.substring(0, 7)
					) {
				...
				if (colorDisplay) {
					colorDisplay.addEventListener("click", (e) => {
						...
						paletteContainer.style.display = isVisible
							? "none"
							: "flex";
					});
				}
			...
			function handleWheel(e) {
				...
				viewOffset.x =
					mouseX - (mouseX - viewOffset.x) * (newZoom / zoom);
				viewOffset.y =
					mouseY - (mouseY - viewOffset.y) * (newZoom / zoom);
				...
				elements = elements.filter((el) => {
					...
					switch (el.type) {
						...
						case "arrow":
							...
							shouldErase =
								dist <= eraseRadius + el.strokeWidth / 2;
							break;

						case "rectangle":
							shouldErase = isPointInRectangle(
								x,
								y,
								el,
								eraseRadius + el.strokeWidth / 2
							);
							break;
						...
						case "circle":
							...
							const radius =
								Math.sqrt(
									el.width * el.width + el.height * el.height
								) / 2;
